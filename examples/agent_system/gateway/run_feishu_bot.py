...
def _get_user(user_id: str) -> dict:
    ...
    user.setdefault("repo_path", _repo_path)
    user.setdefault("current_branch", "")
    return user
...
def _handle_repo(client: lark.Client, ctx: ChatContext, args: list[str]) -> None:
    """Set working directory for git operations. Auto-create if needed."""
    global _repo_path
    import os
    import subprocess
    ...
    if not args:
        _reply(
            client, ctx, f"当前工作目录: {_repo_path}\n用法: /repo <路径或github-url>"
        )
        return
    ...
    if (
        ...
        if os.path.exists(target_path):
            _reply(client, ctx, f"目录已存在: {target_path}\n请先删除或移动后重试")
            return
    ...
    if not os.path.isdir(path):
        # Ask if user wants to create a new git repo or clone
        _reply(
            client,
            ctx,
            f"目录不存在: {path}\n\n"
...
def _handle_commit(client: lark.Client, ctx: ChatContext, args: list[str]) -> None:
    """Commit staged changes. Auto-generates message if not provided."""
    user = _get_user(ctx.user_id)
    repo_path = user.get("repo_path", _repo_path)
    ...
    try:
        ...
        # If no message provided, auto-generate from diff
        if not args:
            # Get git diff
            result = subprocess.run(
            ...
            diff_stat = result.stdout.strip() if result.stdout else "无 staged 文件"

            if diff_stat == "无 staged 文件" or not diff_stat:
            ...
            prompt = f"""根据以下Git变更生成一个简洁的commit message:

```
{diff_stat}
```

生成符合 conventional commits 格式的消息，如: feat: 添加用户登录功能
只输出消息文本，不要其他内容。"""

            result = subprocess.run(
                ["opencode", "run", "--model", "minimax/MiniMax-M2.5", prompt],
                cwd=repo_path,
                capture_output=True,
                text=True,
                timeout=30,
            ...
            if not message or len(message) > 100:
                message = (
                    f"chore: 更新 {diff_stat.split(',')[0] if diff_stat else '代码'}"
                )
        else:
            message = " ".join(args)
...
def _handle_pr(client: lark.Client, ctx: ChatContext, args: list[str]) -> None:
    ...
    try:
        ...
        _reply(client, ctx, "gh CLI未安装，请安装GitHub CLI: brew install gh")
    except Exception as exc:
...
def _handle_merge(client: lark.Client, ctx: ChatContext, args: list[str]) -> None:
    ...
    if not args:
        _reply(
            client,
            ctx,
            "用法: /merge <PR编号或链接>\n示例: /merge 123 或 /merge https://github.com/owner/repo/pull/123",
        )
        return
    pr_input = " ".join(args)
    import re
    ...
    if not pr_match:
        _reply(client, ctx, "请提供PR编号或链接，例如: /merge 123")
        return
