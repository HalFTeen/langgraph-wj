...
def _run_task(thread_id: str, task: str) -> None:
    ...
    if _graph is None:
        _task_results[thread_id] = TaskResult(
            thread_id=thread_id, task=task, status="error: graph not initialized",
        )
        return
    import time
    initial_state: AgentState = {
        "messages": [
            SystemMessage(content="You are a multi-role coding agent. Follow reviewer feedback."),
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            values = state.values
            approval_store.create(ApprovalRequest(
                thread_id=thread_id,
            ...
            print(f"[task:{thread_id}] awaiting approval (iterations={values.get('iteration_count')})")
            return  # success
        except Exception as exc:
            err_str = str(exc)
            is_rate_limit = "429" in err_str or "Too Many Requests" in err_str or "timed out" in err_str or "ReadTimeout" in err_str
            if is_rate_limit and attempt < max_retries:
                wait = attempt * 5  # 5s, 10s, 15s, 20s
                print(f"[task:{thread_id}] rate limited, retrying in {wait}s (attempt {attempt}/{max_retries})")
                _task_results[thread_id] = TaskResult(
...
def resolve_approval(payload: ApprovalResolution) -> ApprovalRecord:
    ...
    if payload.thread_id in _task_results:
        _task_results[payload.thread_id].status = "completed" if status == "approved" else "denied"
        code_files = values.get("code_files", {})
        _task_results[payload.thread_id].code = next(iter(code_files.values()), None) if code_files else None
    return updated
...
def start_pipeline(payload: PipelineStartRequest) -> PipelineStatus:
        ...
        try:
            ...
            for event in compiled.stream(state, config):
                for node_name, node_output in event.items():
                    ...
                    print(f"[pipeline:{thread_id}] node={node_name} phase={node_output.get('phase', '?')}")
            ...
            if agent_result and not agent_result.get("success"):
                final["error"] = agent_result.get("message", "") or agent_result.get("test_output", "")
        except Exception as exc:
            tb = traceback.format_exc()
    ...
    t.start()
    engine = os.getenv("NAL_ENGINE", "fallback")
    print(f"[pipeline:{thread_id}] started (engine={engine}, work_dir={work_dir or 'none'})")
