"""Inter-agent messaging protocol (EXPERIMENTAL — not integrated into the graph).
...
NOTE: This module is not currently used by the production graph (graph.py,
dynamic_graph.py). LangGraph's built-in ``messages: Annotated[list, add_messages]``
state field handles inter-node communication. This module is retained for
potential future use in scenarios requiring priority queues or typed messaging
beyond what LangGraph state provides.
Key components:
